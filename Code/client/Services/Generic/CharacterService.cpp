...
void CharacterService::OnCharacterSpawn(const CharacterSpawnRequest& acMessage) const noexcept
{
    ...
    if (acMessage.FormId == GameId{})
    {
        ...
        const char* pName = pNpc ? pNpc->fullName.value.AsAscii() : "";
#endif
        spdlog::info(__FUNCTION__ ": creating actor, base formID {:X}, IsPlayer() {}, name \"{}\"", pNpc->formID, acMessage.IsPlayer, pName);
...
#if TP_SKYRIM64
    pName = pNpc ? static_cast<const char*>(pNpc->fullName.value) : "";
#elif TP_FALLOUT4
    pName = pNpc ? pNpc->fullName.value.AsAscii() : "";
#endif 
    spdlog::info(__FUNCTION__ ": post SetActorValues() pActor {:X}, formID {:X}, IsPlayer() {}, IsRemote() {}, name \"{}\"", 
                    (uintptr_t)pActor, pActor->formID, 
                    pActor->GetExtension()->IsPlayer(),
                    pActor->GetExtension()->IsRemote(),
                    pName);
